...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/rstack-cli-');
    const execPath = path.join(root, '.github/skills/rstack/browse/dist/browse');
    const serverPath = path.join(root, '.github/skills/rstack/browse/src/server.ts');
