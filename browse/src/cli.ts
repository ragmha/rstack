/**
 * rstack CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
...
export function resolveServerScript(
  ...
  return path.resolve(env.HOME || '/tmp', '.copilot/skills/rstack/browse/src/server.ts');
}
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`rstack browse — Fast headless browser for AI coding agents
